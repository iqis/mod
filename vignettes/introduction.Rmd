...
You do things the R way. For example, `source_module()` and `module()` only returns a module object; It is up to you to put it into your working environemnt with `<-`.d 
...
It is imperative that `mod` be only adopted in the simplest cases. If full-featured OOP is desired, use [`R6`](https://CRAN.R-project.org/package=R6). The user is further warned that OOP, in general, should be used sparingly and with caution, especially during data analysis.
